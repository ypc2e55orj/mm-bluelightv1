...
#define JSON_READ_NUMBER_ARRAY(root, field, len)                                                                       \
  ...
  {                                                                                                                    \
    ...
    {                                                                                                                  \
      size_t i = 0;                                                                                                    \
      for (cJSON *item = array->child; i < (len) && item != nullptr; i++, item = item->next)                           \
      {                                                                                                                \
        if (!cJSON_IsNumber(item))                                                                                     \
        {                                                                                                              \
        ...
        {                                                                                                              \
          field[i] = static_cast<std::remove_reference_t<decltype(*field)>>(item->valuedouble);                        \
        }                                                                                                              \
      }                                                                                                                \
    }                                                                                                                  \
  } while (0)
...
#define JSON_WRITE_NUMBER_ARRAY(root, field, len)                                                                      \
  ...
  {                                                                                                                    \
    ...
    {                                                                                                                  \
      ...
      {                                                                                                                \
        if (!cJSON_AddItemToArray(array, cJSON_CreateNumber(static_cast<double>(field[i]))))                           \
        {                                                                                                              \
