...
class Queue {
 ...
 public:
  ...
  // 送信
  bool send(const T* item, TickType_t xTicksToWait) {
    return xQueueSend(queue_, item, xTicksToWait) == pdTRUE;
  }
  ...
  // 受信
  bool receive(T* const item, TickType_t xTicksToWait) {
    return xQueueReceive(queue_, item, xTicksToWait) == pdTRUE;
  }
  bool receive_isr(T* const item, BaseType_t* pxHigherPriorityTaskWoken) {
    return xQueueReceiveFromISR(queue_, item, pxHigherPriorityTaskWoken) ==
           pdTRUE;
  }

  // 先頭アイテムを取得
  bool peek(T* item, TickType_t xTicksToWait) {
    return xQueuePeek(queue_, item, xTicksToWait) == pdTRUE;
  }
