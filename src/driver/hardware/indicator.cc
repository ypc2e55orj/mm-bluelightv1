...
class RmtIndicator {
 private:
  ...
  // コールバック
  static size_t rmt_indicator_encode(rmt_encoder_t *encoder,
    ...
    switch (indicator_encoder->state) {
      ...
      case 1:  // リセットコードを送信
        encoded_symbols += copy_encoder->encode(
            copy_encoder, channel, &indicator_encoder->reset_code,
 ...
 public:
  explicit RmtIndicator(gpio_num_t indicator_num, uint16_t led_counts,
    ...
    encoder_ = reinterpret_cast<IndicatorEncoder *>(
        heap_caps_calloc(1, sizeof(IndicatorEncoder), MALLOC_CAP_DMA));
    ...
    bytes_encoder_config.bit0.duration1 =
        static_cast<uint16_t>(0.9 * bit_ticks);
    bytes_encoder_config.bit1.level0 = 1;
    bytes_encoder_config.bit1.duration0 =
        static_cast<uint16_t>(0.9 * bit_ticks);
  ...
  bool update() {
    ...
    esp_err_t transmit_err = rmt_transmit(channel_, &encoder_->base, buffer_,
                                          buffer_size_, &tx_config);
...
class Indicator::IndicatorImpl final : public DriverBase {
 ...
 public:
  ...
  void set(size_t pos, uint32_t rgb) { indicator_.set(pos, rgb); }
  void clear() { indicator_.clear(); }
...
void Indicator::set(size_t pos, uint32_t rgb) { return impl_->set(pos, rgb); }
void Indicator::clear() { return impl_->clear(); }
