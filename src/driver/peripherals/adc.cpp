    ...
    switch (unit) {
      ...
      default:
        throw std::runtime_error(
            "Error Adc::AdcImpl(): ADC unit must be specified");
    }
...
Adc::Adc(adc_unit_t unit, adc_channel_t channel)
    : impl_(new AdcImpl(unit, channel)) {}
Adc::~Adc() = default;

int Adc::read() { return impl_->read(); }
bool Adc::read_isr(int &raw) { return impl_->read_isr(raw); }
int Adc::to_voltage() { return impl_->to_voltage(); }
}  // namespace driver::peripherals
