...
namespace task
{
  ...
  public:
    explicit Task(const char *name, TickType_t tick)
      : name_(name), task_(nullptr), tick_(tick), notify_dest_(nullptr), req_stop_(false)
    {
    }
    ...
    {
      req_stop_ = false;
      auto ret = xTaskCreatePinnedToCore(task, name_, usStackDepth, this, uxPriority, &task_, xCoreID);
      return ret == pdTRUE;
    }
    // タスク終了
    bool stop()
